...
const { createTimeout, removeTimeout } = process.binding('timers');

let count = 1; // next timer ID.
...
function setTimeout(callback, delay) {
  ...
  // Return timer's ID.
  return createTimeout(count++, callback, delay, args, false);
}
...
function setInterval(callback, delay) {
  ...
  // Return timer's ID.
  return createTimeout(count++, callback, delay, args, true);
}
