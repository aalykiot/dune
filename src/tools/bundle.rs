...
pub fn run_bundle(entry: &str, skip_cache: bool, minify: bool) -> Result<String> {
    ...
    // Prepend output with dune's message.
    source.insert_str(0, "// This code was bundled using `dune bundle` and it's not recommended to edit it manually\n\n");
    source.insert_str(0, &format!("// Dune v{}\n", env!("CARGO_PKG_VERSION")));
...
impl Resolve for Resolver {
    fn resolve(&self, base: &FileName, specifier: &str) -> Result<FileName, Error> {
        ...
        Ok(FileName::Real(
            Path::new(&resolve_import(base, specifier)?).to_path_buf(),
        ))
    }
}
