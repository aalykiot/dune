...
lazy_static! {
    static ref VERSIONS: HashMap<&'static str, &'static str> = {
        ...
        map.insert("pluto", env!("CARGO_PKG_VERSION"));
...
pub fn initialize<'s>(
    ...
    //  when the pluto process was launched.
    ...
    // `process.version` - the pluto version.
    let version = format!("v{}", VERSIONS.get("pluto").unwrap());
    let version = v8::String::new(scope, version.as_str()).unwrap();
    set_property_to(scope, process, "version", version.into());

    // `process.versions` - an object listing the version strings of pluto and its dependencies.
